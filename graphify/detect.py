...
# Output directory — override with GRAPHIFY_OUT env var for custom output locations
...
_SKIP_DIRS = {
    ...
    ".tox", ".eggs", "*.egg-info",
    # Note: graphify output directory is handled dynamically in _is_noise_dir()
}
...
def _is_noise_dir(part: str) -> bool:
    ...
    if part in _SKIP_DIRS:
        return True
    # Check if this matches the graphify output directory (could be custom via GRAPHIFY_OUT)
    if part == _GRAPHIFY_OUT or part == os.path.basename(_GRAPHIFY_OUT):
        return True
...
def detect(root: Path, *, follow_symlinks: bool = False) -> dict:
    ...
    # Always include memory/ directory - query results filed back into the graph
