...
/graphify <path> --out custom-output                  # write output to custom-output/ instead of graphify-out/
...
**Custom output directory:** If the user passed `--out <directory>`, set the environment variable `GRAPHIFY_OUT=<directory>` in all subsequent bash commands. This overrides the default `graphify-out/` location. The directory can be a relative path (e.g., `docs/graph`) or an absolute path (e.g., `/shared/project-graph`).
...
# Use custom output directory if GRAPHIFY_OUT is set
