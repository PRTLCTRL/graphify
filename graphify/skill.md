---
name: graphify
description: "any input (code, docs, papers, images, videos) to knowledge graph. Use when user asks any question about a codebase, documents, or project content - especially if $OUTPUT_DIR/ exists, treat the question as a /graphify query."
trigger: /graphify
---
...
/graphify <path> --neo4j                              # generate $OUTPUT_DIR/cypher.txt for Neo4j
...
graphify merge-graphs \
  ...
  --out $OUTPUT_DIR/cross-repo-graph.json
```
...
**Strategy:** Read the god nodes from `$OUTPUT_DIR/.graphify_detect.json` (or the analysis file if it exists from a previous run). You are already a language model — write a one-sentence domain hint yourself from those labels. Then pass it to Whisper as the initial prompt. No separate API call needed.
...
if code_files:
    ...
    Path('$OUTPUT_DIR/.graphify_ast.json').write_text(json.dumps(result, indent=2))
    print(f'AST: {len(result[\"nodes\"])} nodes, {len(result[\"edges\"])} edges')
else:
    Path('$OUTPUT_DIR/.graphify_ast.json').write_text(json.dumps({'nodes':[],'edges':[],'input_tokens':0,'output_tokens':0}))
...
- Load `total_words` and file counts from `$OUTPUT_DIR/.graphify_detect.json`
...
if cached_nodes or cached_edges or cached_hyperedges:
    Path('$OUTPUT_DIR/.graphify_cached.json').write_text(json.dumps({'nodes': cached_nodes, 'edges': cached_edges, 'hyperedges': cached_hyperedges}))
Path('$OUTPUT_DIR/.graphify_uncached.txt').write_text('\n'.join(uncached))
print(f'Cache: {len(all_files)-len(uncached)} files hit, {len(uncached)} files need extraction')
"
```

Only dispatch subagents for files listed in `$OUTPUT_DIR/.graphify_uncached.txt`. If all files are cached, skip to Part C directly.

**Step B1 - Split into chunks**

Load files from `$OUTPUT_DIR/.graphify_uncached.txt`. Split into chunks of 20-25 files each. Each image gets its own chunk (vision needs separate context). When splitting, group files from the same directory together so related artifacts land in the same chunk and cross-file relationships are more likely to be extracted.
...
- Check that `$OUTPUT_DIR/.graphify_chunk_NN.json` exists on disk — this is the success signal
...
chunks = sorted(glob.glob('$OUTPUT_DIR/.graphify_chunk_*.json'))
...
Path('$OUTPUT_DIR/.graphify_semantic_new.json').write_text(json.dumps({
...
new = json.loads(Path('$OUTPUT_DIR/.graphify_semantic_new.json').read_text()) if Path('$OUTPUT_DIR/.graphify_semantic_new.json').exists() else {'nodes':[],'edges':[],'hyperedges':[]}
saved = save_semantic_cache(new.get('nodes', []), new.get('edges', []), new.get('hyperedges', []))
print(f'Cached {saved} files')
"
```

Merge cached + new results into `$OUTPUT_DIR/.graphify_semantic.json`:
```bash
$(cat $OUTPUT_DIR/.graphify_python) -c "
import json
from pathlib import Path

cached = json.loads(Path('$OUTPUT_DIR/.graphify_cached.json').read_text()) if Path('$OUTPUT_DIR/.graphify_cached.json').exists() else {'nodes':[],'edges':[],'hyperedges':[]}
new = json.loads(Path('$OUTPUT_DIR/.graphify_semantic_new.json').read_text()) if Path('$OUTPUT_DIR/.graphify_semantic_new.json').exists() else {'nodes':[],'edges':[],'hyperedges':[]}
...
Path('$OUTPUT_DIR/.graphify_semantic.json').write_text(json.dumps(merged, indent=2))
print(f'Extraction complete - {len(deduped)} nodes, {len(all_edges)} edges ({len(cached[\"nodes\"])} from cache, {len(new.get(\"nodes\",[]))} new)')
"
```
Clean up temp files: `rm -f $OUTPUT_DIR/.graphify_cached.json $OUTPUT_DIR/.graphify_uncached.txt $OUTPUT_DIR/.graphify_semantic_new.json`
...
ast = json.loads(Path('$OUTPUT_DIR/.graphify_ast.json').read_text())
sem = json.loads(Path('$OUTPUT_DIR/.graphify_semantic.json').read_text())
...
Path('$OUTPUT_DIR/.graphify_extract.json').write_text(json.dumps(merged, indent=2))
...
extraction = json.loads(Path('$OUTPUT_DIR/.graphify_extract.json').read_text())
detection  = json.loads(Path('$OUTPUT_DIR/.graphify_detect.json').read_text())
...
Path('$OUTPUT_DIR/GRAPH_REPORT.md').write_text(report)
...
Path('$OUTPUT_DIR/.graphify_analysis.json').write_text(json.dumps(analysis, indent=2))
...
Read `$OUTPUT_DIR/.graphify_analysis.json`. For each community key, look at its node labels and write a 2-5 word plain-language name (e.g. "Attention Mechanism", "Training Pipeline", "Data Loading").
...
extraction = json.loads(Path('$OUTPUT_DIR/.graphify_extract.json').read_text())
detection  = json.loads(Path('$OUTPUT_DIR/.graphify_detect.json').read_text())
analysis   = json.loads(Path('$OUTPUT_DIR/.graphify_analysis.json').read_text())
...
Path('$OUTPUT_DIR/GRAPH_REPORT.md').write_text(report)
Path('$OUTPUT_DIR/.graphify_labels.json').write_text(json.dumps({str(k): v for k, v in labels.items()}))
...
- If `--obsidian-dir <path>` was also given, use that path as the vault directory. Otherwise default to `$OUTPUT_DIR/obsidian`.
...
extraction = json.loads(Path('$OUTPUT_DIR/.graphify_extract.json').read_text())
analysis   = json.loads(Path('$OUTPUT_DIR/.graphify_analysis.json').read_text())
labels_raw = json.loads(Path('$OUTPUT_DIR/.graphify_labels.json').read_text()) if Path('$OUTPUT_DIR/.graphify_labels.json').exists() else {}
...
obsidian_dir = 'OBSIDIAN_DIR'  # replace with --obsidian-dir value, or '$OUTPUT_DIR/obsidian' if not given
...
if G.number_of_nodes() > NODE_LIMIT:
    ...
    if meta.number_of_nodes() > 1:
        ...
        to_html(meta, meta_communities, '$OUTPUT_DIR/graph.html', community_labels=labels or None, member_counts=member_counts)
        print(f'graph.html written (aggregated: {meta.number_of_nodes()} community nodes, {meta.number_of_edges()} cross-community edges)')
        print('Tip: run with --obsidian for full node-level detail.')
    else:
        print('Single community — aggregated view not useful. Skipping graph.html.')
else:
    to_html(G, communities, '$OUTPUT_DIR/graph.html', community_labels=labels or None)
...
n = to_wiki(G, communities, '$OUTPUT_DIR/wiki', community_labels=labels or None, cohesion=cohesion, god_nodes_data=gods)
print(f'Wiki: {n} articles written to $OUTPUT_DIR/wiki/')
print('  $OUTPUT_DIR/wiki/index.md  ->  agent entry point')
"
```
...
G = build_from_json(json.loads(Path('$OUTPUT_DIR/.graphify_extract.json').read_text()))
to_cypher(G, '$OUTPUT_DIR/cypher.txt')
print('cypher.txt written - import with: cypher-shell < $OUTPUT_DIR/cypher.txt')
"
```
...
to_svg(G, communities, '$OUTPUT_DIR/graph.svg', community_labels=labels or None)
...
to_graphml(G, communities, '$OUTPUT_DIR/graph.graphml')
...
```bash
python3 -m graphify.serve $OUTPUT_DIR/graph.json
```
...
If `total_words` from `$OUTPUT_DIR/.graphify_detect.json` is greater than 5,000, run:
...
detection = json.loads(Path('$OUTPUT_DIR/.graphify_detect.json').read_text())
result = run_benchmark('$OUTPUT_DIR/graph.json', corpus_words=detection['total_words'])
...
extract = json.loads(Path('$OUTPUT_DIR/.graphify_extract.json').read_text())
...
"
rm -f $OUTPUT_DIR/.graphify_detect.json $OUTPUT_DIR/.graphify_extract.json $OUTPUT_DIR/.graphify_ast.json $OUTPUT_DIR/.graphify_semantic.json $OUTPUT_DIR/.graphify_analysis.json $OUTPUT_DIR/.graphify_chunk_*.json
rm -f $OUTPUT_DIR/.needs_update 2>/dev/null || true
```

Tell the user (omit the obsidian line unless --obsidian was given):
```
Graph complete. Outputs in PATH_TO_DIR/$OUTPUT_DIR/
...
```bash
if [ ! -f $OUTPUT_DIR/.graphify_python ]; then
    GRAPHIFY_BIN=$(which graphify 2>/dev/null)
...
Path('$OUTPUT_DIR/.graphify_incremental.json').write_text(json.dumps(result))
if new_total == 0:
...
result = json.loads(open('$OUTPUT_DIR/.graphify_incremental.json').read()) if Path('$OUTPUT_DIR/.graphify_incremental.json').exists() else {}
...
# Load existing graph
existing_data = json.loads(Path('$OUTPUT_DIR/graph.json').read_text())
G_existing = json_graph.node_link_graph(existing_data, edges='links')

# Load new extraction
new_extraction = json.loads(Path('$OUTPUT_DIR/.graphify_extract.json').read_text())
G_new = build_from_json(new_extraction)

# Prune nodes from deleted files
incremental = json.loads(Path('$OUTPUT_DIR/.graphify_incremental.json').read_text())
...
Path('$OUTPUT_DIR/.graphify_extract.json').write_text(json.dumps(merged_out))
...
old_data = json.loads(Path('$OUTPUT_DIR/.graphify_old.json').read_text()) if Path('$OUTPUT_DIR/.graphify_old.json').exists() else None
new_extract = json.loads(Path('$OUTPUT_DIR/.graphify_extract.json').read_text())
...
Before the merge step, save the old graph: `cp $OUTPUT_DIR/graph.json $OUTPUT_DIR/.graphify_old.json`
Clean up after: `rm -f $OUTPUT_DIR/.graphify_old.json`
...
data = json.loads(Path('$OUTPUT_DIR/graph.json').read_text())
...
$(cat $OUTPUT_DIR/.graphify_python) -c "
from pathlib import Path
if not Path('$OUTPUT_DIR/graph.json').exists():
...
```bash
$(cat $OUTPUT_DIR/.graphify_python) -m graphify save-result --question "QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
...
```bash
$(cat $OUTPUT_DIR/.graphify_python) -m graphify save-result --question "Path from NODE_A to NODE_B" --answer "ANSWER" --type path_query --nodes NODE_A NODE_B
```
...
```bash
$(cat $OUTPUT_DIR/.graphify_python) -m graphify save-result --question "Explain NODE_NAME" --answer "ANSWER" --type explain --nodes NODE_NAME
```
