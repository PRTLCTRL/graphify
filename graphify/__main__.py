...
def main() -> None:
    # Check for --output-dir early and set environment variable before any imports
    # This ensures all modules that read GRAPHIFY_OUT get the updated value
    i = 1  # Start from index 1 (skip program name)
    while i < len(sys.argv):
        if sys.argv[i] in ("--output-dir", "-o") and i + 1 < len(sys.argv):
            os.environ["GRAPHIFY_OUT"] = sys.argv[i + 1]
            sys.argv.pop(i)  # Remove flag
            sys.argv.pop(i)  # Remove argument value (now at position i)
            break
        elif sys.argv[i].startswith("--output-dir="):
            os.environ["GRAPHIFY_OUT"] = sys.argv[i].split("=", 1)[1]
            sys.argv.pop(i)  # Remove flag with value
            break
        i += 1
    
    # Update the module-level variable after potentially setting env var
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help"):
        print("Usage: graphify <command> [--output-dir DIR]")
        print()
        print("Global Options:")
        print("  --output-dir DIR, -o DIR  specify custom output directory (default: graphify-out)")
