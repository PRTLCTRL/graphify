...
The path supports:
- Environment variables: `$VAR` or `${VAR}` on Unix, `%VAR%` on Windows (e.g., `%USERPROFILE%/project/graphify-out/graph.json`)
...
- Absolute paths: `C:/absolute/path/to/graphify-out/graph.json`
