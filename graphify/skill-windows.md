...
if deleted:
    ...
    if to_remove:
        print(f'Pruned {len(to_remove)}/{len(deleted)} ghost nodes from deleted file(s) — drift detected and corrected.')
    else:
        print(f'Manifest listed {len(deleted)} deleted file(s); none were present in the graph — no drift.')
