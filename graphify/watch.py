...
def _rebuild_code(watch_path: Path, *, follow_symlinks: bool = False, force: bool = False, output_dir: str | None = None) -> bool:
...
def check_update(watch_path: Path, output_dir: str | None = None) -> bool:
    ...
    """
    _out_dir = output_dir or _GRAPHIFY_OUT
    flag = Path(watch_path) / _out_dir / "needs_update"
    if flag.exists():
...
def _notify_only(watch_path: Path, output_dir: str | None = None) -> None:
    """Write a flag file and print a notification (fallback for non-code-only corpora).
    
    output_dir: custom output directory name/path (overrides env var and default).
    """
    _out_dir = output_dir or _GRAPHIFY_OUT
    flag = watch_path / _out_dir / "needs_update"
...
def watch(watch_path: Path, debounce: float = 3.0, output_dir: str | None = None) -> None:
    """
        ...
        while True:
            ...
            if pending and (time.monotonic() - last_trigger) >= debounce:
                ...
                if _has_non_code(batch):
                    _notify_only(watch_path, output_dir)
                else:
