...
def _rebuild_code(watch_path: Path, *, follow_symlinks: bool = False, force: bool = False) -> bool:
    ...
    try:
        ...
        out = watch_path / _get_output_dir()
...
def check_update(watch_path: Path) -> bool:
    ...
    """
    flag = Path(watch_path) / _get_output_dir() / "needs_update"
    if flag.exists():
...
def _notify_only(watch_path: Path) -> None:
    ...
    flag = watch_path / _get_output_dir() / "needs_update"
    ...
    class Handler(FileSystemEventHandler):
        def on_any_event(self, event):
            ...
            if any(part.startswith(".") for part in path.parts):
                return
            if _get_output_dir() in path.parts:
                return
