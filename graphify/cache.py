...
def cache_dir(root: Path = Path("."), kind: str = "ast", output_dir: str | None = None) -> Path:
    """Returns {output_dir}/cache/{kind}/ - creates it if needed.
...
def load_cached(path: Path, root: Path = Path("."), kind: str = "ast", output_dir: str | None = None) -> dict | None:
    ...
    Cache value: stored as {output_dir}/cache/{kind}/{hash}.json
    ...
    if kind == "ast":
        legacy = Path(root).resolve() / output_dir / "cache" / f"{h}.json"
        if legacy.exists():
            try:
...
def save_cached(path: Path, result: dict, root: Path = Path("."), kind: str = "ast", output_dir: str | None = None) -> None:
    ...
    Stores as {output_dir}/cache/{kind}/{hash}.json where hash = SHA256 of current file contents.
...
def cached_files(root: Path = Path("."), output_dir: str | None = None) -> set[str]:
    ...
    if output_dir is None:
        output_dir = _GRAPHIFY_OUT
    base = Path(root).resolve() / output_dir / "cache"
    hashes: set[str] = set()
    # Legacy flat entries
    if base.is_dir():
